...
The natural map[?] \([X, \bigvee_i \bb{K}(G,n_i)]\to \prod_i [X,\bb{K}(G,n_i)]\) is an isomorphism if \(X\) is a connective spectrum of finite type and \(n_i\to \infty\) as \(i\to\infty\). 
\end{proposition}
...
\begin{itemize}
\item Fact: \(K_0\) has finite type (and thus so do all the \(K_s\)'s)
...
\item Fact: each \(H^*(K_s)\) is a free \(\A\)-module, and thus the diagram above gives a resolution of \(H^*(X)\) by free \(\A\)-modules. 

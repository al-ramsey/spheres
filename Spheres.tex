...
\subsubsection{A Brief Summary, or How to Lose Your Mind Because of One Stupid Prime Number [TO DELETE]}

\begin{itemize}
\item There is a multiplication, the \textit{Yoneda product}, on \(E_2 = \Ext\)(whatever). 

\item There is a multiplication, the \textit{composition product}, on \(\pi_*^S\). It's graded commutative.

\item There is (supposedly!!!) a multiplication, also called the composition product, on \(\text{}_2\pi_*^S\) (whatever that object is). It may be inherited from the one on \(\pi_*^S\), though it's not at all clear how. It should be graded commutative as well.

\item There is a multiplication on \(E_r\) which coincides with the Yoneda product on \(E_2\) and converges (whatever that means) to the composition product on \(\text{}_2\pi_*^S\) (whatever that is). It's associative and unital, and plays nicely with the differentials. 
\end{itemize}

Questions/problems (briefly):
\begin{itemize}
\item Is \(\text{}_2\pi_*^S\) equal to \(\text{}_2 \left(\bigoplus_i \pi_i^S\right)\) (i.e. the completion of the ring), or \(\bigoplus_i \text{}_2\pi_*^S\)?
\item If it's the latter, how does the composition product on \(\pi_*^S\) give rise to a `composition product' on \(\text{}_2\pi_*^S\)?
...
\begin{itemize}
\item 11.8: For spectra \(X, Y, Z\), the composition of morphisms defines a pairing 
\[\circ : [Y, Z]_n \otimes [X,Y]_m \to [X,Z]_{n+m}\]
[doing what it should].

\item For any algebra \(A\) and \(A\)-modules \(L,M,N\), there is a natural Yoneda composition product
...
is the twisted composition product, mapping \(y\otimes x\) to \((-1)^{\abs{x}\abs{y}}x\circ y\), where \(\abs{x}=v-u\) and \(\abs{y}=t-s\) for \(x\in ''E_2^{u,v}\) and \(y \in 'E^{s,t}_2\).
\end{enumerate}
\end{itemize}
...
\subsubsection{The actual chapter [HEADING TO DELETE]}
...
and which converges\footnote{What does that mean? The spectral sequence itself only converges to a filtration of \(\text{}_{2}\pi_*^S\), so is this saying that the composition product (graded commutative) on \(\pi_*^S\) induces (descends to? lifts to?) a product on \(\text{}_2\pi_*^S\), which itself descends to a product on \(E^{s,t}_\infty=F^{s,t}/F^{s+1,t+1}\)? Even if the first part works, why would the product land in the correct subgroup?} to the composition pairing\footnote{That's not a composition pairing!! A composition pairing would be something like \([Y, \text{}_2Z]_*\otimes [X,Y]_* \to [X, \text{}_2Z]_*\) or even \([\text{}_2Y, \text{}_2Z]_* \otimes [X, \text{}_2Y]_* \to [X, \text{}_2Z]_*\), but not what's written up there! How on earth is that thing even defined??}
...
% sweet naive me below didn't notice that the maps were going the wrong way :')
...
\item Somehow[?] \(\text{}_2\pi_*^S\) (by which I mean \(\bigoplus_i \text{}_2\pi_i^S\), NOT \(\text{}_2\bigoplus_i \pi_i^S\)) inherits a ring structure from this, and it coincides exactly with the composition product (in whatever way that makes sense). Here is a way in which it might make sense: we're supposed to get a product

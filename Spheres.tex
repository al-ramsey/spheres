...
\textcolour{teal}{[Some intro. Following \autocite{spectral_sequences}, \autocite{hatcher5}, \autocite{weibel}, \autocite{hatcher}.]}
...
We define \(\Ext^{n}(M,N):=H^n(\Hom_R(P_\bullet, N))=H^n(\Hom_R(N, I_\bullet))\). 
...
We will say that a free resolution is \textit{minimal} if at each stage of its construction we choose the minimal number of free generators for \(F_i\) in each degree.

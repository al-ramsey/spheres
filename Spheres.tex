...
\[A \inj X \to X\cap CA \to \Sigma A \inj \Sigma X \to \cdots.\]
...
Y \arrow[r, "\id"] \arrow[d, swap, "", dashrightarrow]  & Y \arrow[r, "", hookrightarrow] \arrow[d, "f"] & CY \arrow[d, "h"] \arrow[r, "", hookrightarrow] & \Sigma Y \arrow[d, "", dashrightarrow] \arrow[r, "\id"] & \Sigma Y \arrow[d, "\Sigma f", dashrightarrow]\\ 
 ...
 \end{tikzcd}\] 
\textcolour{purple}{[\autocite{hatcher5} says that ``we can then automatically fill in the next two vertical maps to make homotopy commutative squares''. I believe that we can write down a map making the left square commute, but I have no idea why that map makes the right square commute. But if it does...]}
...
% oh my god I may be stupid
Now, we have a map 
\begin{align*}
[\bb{S}, K_s]&\to \Hom_{\A}^0(H^*(K_s))\\
f &\mapsto f^*,
\end{align*}
since \(f^*\) is an \(\A\)-module homomorphism by \ref{2504201153} (1). 
\textcolour{purple}{[Fact: it's an isomorphism.]}
...
\centering
\input{mini_ext.tex}
\caption{\(\Ext^{s,t}_{\A}(\bb{F}_2, \bb{F}_2)\) for \(t-s\leq 5\).}
\label{2504241018}
\end{figure}

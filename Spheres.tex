...
Something I need later: \(d_2(f_0)\) is nontrivial:
\begin{itemize}
\item \(d_2(i)\) is nontrivial, because \(h_4i=0\) and \(h_0h_3^2i\neq 0\), so \(0=d_2(h_4i)=h_0h_3^2i+h_4d_2(i)\) implies \(d_2(i)\neq 0\).
\item \(d_2(j)\neq 0\) since \(h_0d_2(j)=d_2(h_0j)=d_2(h_2i)=h_2d_2(i)\neq 0\).  
\item A similar argument shows \(d_2(k), d_2(l)\neq0\), and \(d_2(h_0l)\neq 0\).
...
\[0 \leftarrow \bb{F}_2 \leftarrow H^*(C \sigma) \leftarrow \Sigma^8 \bb{F}_2 \leftarrow 0.\]

Claim 2: From the short exact sequence above, we get a long exact sequence \(\cdots \to \Ext^{s-1,t-8}(\bb{F}_2, \bb{F}_2) \to \Ext^{s,t}(\bb{F}_2, \bb{F}_2)\to \Ext^{s,t}(H^*(C \sigma), \bb{F}_2) \to \Ext^{s,t-8}(\bb{F}_2, \bb{F}_2)\to \Ext^{s+1,t}(\bb{F}_2, \bb{F}_2)\to \cdots\)
...
\caption{The \(E_2\) page of the Adams spectral sequence for \(C \sigma\), in the range \(t-s\leq 18\).}
\end{figure}

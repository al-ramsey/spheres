...
%Possible lead: on p124 of \autocite{ass}, it says ``Associated to the [cofibre] sequence \(S \to S/2 \to \Sigma S=S^1\), we have an extension \(0 \to \Sigma \bb{F}_2 \to H^*(S/2)\to \bb{F}_2 \to 0\) and a long exact sequence of \(\Ext\)-groups \(\cdots \to \Ext^{s,t}_{\A}(\bb{F}_2, \bb{F}_2)\to \Ext^{s,t}(H^*(S/2), \bb{F}_2)\to \Ext^{s, t-1}_{\A}(\bb{F}_2, \bb{F}_2)\xrightarrow{\delta}\Ext^{s+1,t}_{\A}(\bb{F}_2, \bb{F}_2)\to \cdots\)''. So: cofibration sequences do seem to give maps on Ext, though it's never said why. 

...
\begin{itemize}
\item Fact: \(K_0\) has finite type (and thus so do all the \(K_s\)'s) [I'm sure this is supposed to follow from \(H^i(X)\) being finitely generated for all \(i\), but I can't at all see how].
\item Fact: the top row of the diagram above is exact. [Suffices to show that we have exact sequences: 
...
\item Fact: we get a staircase diagram
...
\item Fact: this gives rise to a spectral sequence.

\item Fact: elements of \([\bb{S}, K_s]\) are tuples of elements of \(H^*(\bb{S})\), one for each summand \(K_{s,i}\) in the appropriate group \(H^{n_i}(\bb{S})\).

\item Def: \(\Hom^t_{\A}\).

\item Fact: there is a natural map \([\bb{S}, K_s]\to \Hom^0_{\A}(H^*(K_s), H^*(\bb{S}))\), and it's an isomorphism.

\item Fact: there is a `natural identification'
\[[\Sigma^t \bb{S}]=\Hom^0_{\A}(H^*(K_s), H^*(\Sigma^t \bb{S}))=\Hom^t_{\A}(H^*(K_s), H^*(\bb{S})).\]

\item Fact: \(E_1^{s,t}=\Hom^t_{\A}(H^*(K_s), H^*(\bb{S}))\).

\item Fact: the differential \(d_1 : \pi_t(K_s)\to\pi_t K_{s+1}\) is induced by the map \(K_s \to K_{s+1}\). 

\item Fact: the \(E_1\) page of the spectral sequence `consists of'[??] the complexes 
\[0 \to \Hom^t_{\A}(H^*(K_0), H^*(\bb{S}))\to \Hom^t_{\A}(H^*(K_1), H^*(\bb{S}))\to \cdots\]

\item Fact: \(E_2^{s,t}=\Ext^{s,t}_{\A}(H^*(X), H^*(\bb{S}))\). 
\end{itemize}
...
There is a spectral sequence \(\{E_r, d_r\}\) such that \(E_2^{s,t}=\Ext_{\A}^{s,t}(\bb{F}_2, \bb{F}_2)\) and \(\{E_r, d_r\}\implies \pi_*^S\) modulo torsion of odd order. [which is probably a lie but let's worry about that later]
\end{theorem}
...
It satisfies the Leibniz rule, and it converges to the composition product\footnote{What does that mean? The spectral sequence itself only converges to the associated graded(?) of \(\text{}_{2}\pi_*^S\), so is this saying that the composition product (graded commutative) on \(\pi_*^S\) induces (descends to? lifts to?) a product on \(\text{}_2\pi_*^S\), which itself descends to a product on \(E^{s,t}_\infty=F^{s,t}/F^{s+1,t+1}\)? Even if the first part works, why would the product land in the correct subgroup?} on \(\text{}_2\pi_*^S\). 
...
\subsubsection{...that probably do matter}

...
\textcolour{teal}{[Intro. Sources: \autocite{hatcher}]}
...
\begin{proposition}[{\autocite{hatcher}, p496}]
...
The admissible monomials are exactly those to which no Adem relations can be applied. Thus, \(\A\) is generated as a \(\inte/2\inte\) module by admissible monomials. 

\section{Spectra may not be your friends, but I can introduce you}

\textcolour{teal}{[Intro. Sources: \autocite{ass}, \autocite{hatcher5}, \autocite{concise}, \autocite{spectra}]}
...
\textcolour{teal}{[Some intro, below follows \autocite{hatcher5}, Section 5.2. Some intuition on spectra and why we're doing this.]}
...
\nathaniel{Important example:

At the bottom of p592 of \autocite{hatcher5}, it's mentioned that for a spectrum \(X\) of finite type, \(H^i(X)\cong \lim\limits_{\leftarrow n}H^{i+n}(X_n)\) as \(\mathscr{A}_2\)-modules\footnote{\textcolour{teal}{Maybe I need to move this somewhere else. I haven't  defined the \(\A\)-action yet...}}. Thus, since \(\bb{S}\) is of finite type (it is actually finite), we have
\begin{align*}
...
\textcolour{teal}{[Some intuition: it's enough for maps to be `defined eventually']}
...
\textcolour{teal}{[Some intro. Following \autocite{spectral_sequences}, \autocite{hatcher5}]}
...
\textcolour{teal}{[Following \autocite{spectral_sequences}, C2]}
...
\begin{itemize}
\item[\textcolour{purple}{\textbullet}] \textcolour{purple}{Fact: \(K_0\) has finite type (and thus so do all the \(K_s\)'s) [I'm sure this is supposed to follow from \(H^i(X)\) being finitely generated for all \(i\), but I can't at all see how].}
\item[\textcolour{purple}{\textbullet}] \textcolour{purple}{Fact: the top row of the diagram above is exact.} \purple{Suffices to show that we have exact sequences: 
...
This gives rise to a spectral sequence, by \textcolour{teal}{[some version of]} \ref{2503301131}.
...
\textcolour{teal}{[Def: \(\Hom^t_{\A}\) (it's just the homs that lower the degree by \(t\))]}

\textcolour{purple}{[Fact: there is a natural map \([\bb{S}, K_s]\to \Hom^0_{\A}(H^*(K_s), H^*(\bb{S}))\), and it's an isomorphism. [??]]}

We thus have
\[[\Sigma^t \bb{S}, K_s]=\Hom^0_{\A}(H^*(K_s), H^*(\Sigma^t \bb{S}))=\Hom^t_{\A}(H^*(K_s), H^*(\bb{S})).\]
In the case of CW complexes, we have \(H^*(\Sigma^t X)\cong H^{*-t}(X)\). Since \(\bb{S}\) has finite type, for \(i\) large enough we have \(H^n(\Sigma^t\bb{S})=H^{n+i}(\Sigma^t S^i)\cong H^{n+i-t}(S^i)=H^{n-t}(\bb{S})\).

Now, \(E_1^{s,t} = \pi_t K_s =\Hom^t_{\A}(H^*(K_s), H^*(\bb{S}))\), since the staircase diagram comes from \textcolour{teal}{(or gives rise to, depending on your point of view)} the exact couple:
\[\begin{tikzcd}
...
\begin{figure}\label{2504201106}
\input{table.tex}
\caption{\textcolour{teal}{Table}}
\end{figure}

\textcolour{violet}{[Explain the table in Figure \ref{2504201106}]}

From Figure \ref{2504201106}, we see that \((\pi^S_3)^\wedge_2\) has order \(8\). However, we do not currently have the tools to determine whether it is isomorphic to \((\inte/2\inte)^3\), \(\inte/2\inte\oplus \inte/4\inte\), or \(\inte/8\inte\). We will therefore spend some time describing a multiplication on the Adams spectral sequence which will allow us to resolve such ambiguities.

\subsection{Multiplicative structure}

\textcolour{teal}{[Intro. Sources: \autocite{ass}, \autocite{rognes2}, \autocite{hatcher}]}
...
be free resolutions for \(L\) and \(M\). Then, given \([g] \in \Ext^{s,t}(M, N)\), \([h]\in \Ext^{u,v}(L, M)\), we inductively construct a chain map \(h_\bullet : F_{u+\bullet} \to F'_{\bullet}[v]\), as shown in the diagram below.
\[\begin{tikzcd} 
...
& N[v+t]
\end{tikzcd}\]
We have \(gh_sf_{u+s+1}=gf'_{s+1}h_{s+1}=0\), since \([g] \in \Ext^{s,t}(F'_s, N)\), so \([gh_s]\in \Ext^{u+s, v+t}\). We thus define \([g]\cdot  [h]=[gh_s]\). 
\end{definition}
...
\nathaniel{Some intro. Sources: \autocite{ass}, \autocite{rognes2}}
...
\textcolour{teal}{(Proof/calculation)}
...
\subsection{The \(E_2\) page for \(t-s\leq 15\)}

\begin{figure}\label{1304251243}
\centering
\input{ext.tex}
\caption[\(\Ext_{\A}^{s,t}(\bb{F}_2, \bb{F}_2)\) for \(t-s\leq 18\).]{\(\Ext_{\A}^{s,t}(\bb{F}_2, \bb{F}_2)\) for \(t-s\leq 18\). The vertical and diagonal lines indicate multiplication by \(h_0\) and \(h_1\) respectively. Some of the algebra generators are shown in red, with their standard names.}
\end{figure}
...
\begin{ourproof}
First, note that the only possible nontrivial differentials in this range are \(d_r : E_r^{1,2}(\bb{S}) \to E_r^{1+r, 1+r}(\bb{S})\) and \(d_2 : E_2^{2, 10}(\bb{S}) \to E_2^{4, 11}(\bb{S})\). 

Now, \(0=d_r(h_0h_1)=d_r(h_0)h_1 + h_0d_r(h_1)=h_0d_r(h_1)\), so \(d_r(h_1)=0\). Since \(E_r^{1, 2}(\bb{S})\) is generated by \(h_1\), we must have \(d_r=0\). 

On the other hand, \(E^{2, 10}_2(\bb{S})\) is generated by \(h_1h_3\), and \(d_2(h_1h_3)=d_2(h_1)h_3+h_1d_2(h_3)=0+0=0\) (the first factor is zero by the previous computation, and the second is an element of a trivial group). \done
\end{ourproof}

\subsection{Differentials at \(14\leq t-s\leq 15\)}

\nathaniel{[The point here is that all differentials interacting with \(E^{s,t}\) for \(t-s<14\) are trivial, and thus computing the stable homotopy groups is purely mechanical, because everything that appears on this part of the \(E_2\) page has to survive to \(E_\infty\). Thus, the `ambiguity' at \(t-s=14\) is just the fact that this is the first time you need to actually compute differentials.]}
...
\[(f^*)^* : \Ext_{\A}^{s,t}(H^*(Y), \bb{F}_2)\to \Ext^{s,t}_{\A}(H^*(Z), \bb{F}_2)\]
satisfies \((f^*)^*(\alpha \beta)=(f^*)^*(\alpha)(f^*)^*(\beta)\). This follows from the definition of the Yoneda product, since \(f^*\) induces a chain homotopy between resolutions of \(H^*(Z)\) and \(H^*(Y)\), so both \((f^*)^*(\alpha \beta)\) and \((f^*)^*(\alpha)(f^*)^*(\beta)\) arise from chain homotopies, and thus descend to same element in \(\Ext^{*,*}(H^*(Z), \bb{F}_2)\).
\end{remark}
...
\begin{landscape}
\begin{figure}\label{2504201924}
\input{extra.tex}
\caption{\(\Ext^{s,t}_{\A}(\bb{F}_2, \bb{F}_2)\) for \(t-s\leq 32\). The vertical and diagonal lines indicate multiplication by \(h_0\) and \(h_1\) respectively. Some of the algebra generators are shown in red, with their standard names. The \(d_2\) differentials referenced in the proof of \ref{2504192017} are shown in green.}
\end{figure}
\end{landscape}
...
Let \(\overline{\overline{h_0^2h_3}}\in E_2^{3,18}(C_\sigma)\) be the generator shown in Figure \ref{2504131812}. Then \(d_2(\overline{\overline{h_0^2h_3}})=\hat i(h_0d_0)\), where \(\hat i = (i^*)^* : \Ext^{s,t}(\bb{F}_2, \bb{F}_2)\to \Ext^{s,t}(C_\sigma, \bb{F}_2)\) is the map induced by \(i^* : H^*(C_\sigma)\to H^*(\bb{S})\). %filling in the diagram below with the unique dotted lifts.\footnote{Make this explanation make any sense.}
%\[\begin{tikzcd} 
...
and from this we get\footnote{\textcolour{teal}{This is a homological algebra fact, but maybe I should state it somewhere - trouble is I'd have to define Ext in terms of injective resolutions instead of projective ones...}} 
% Issue with the below is that I defined Ext in terms of projective (well, actually, free) resolutions, instead of injective resolutions. Oops!
%a short exact sequence of chain complexes
%\[0 \to \Hom(\bb{F}_2, I_\bullet) \xrightarrow{i^*} \Hom(H^*(C_\sigma), I_\bullet) \xrightarrow{j^*} \Hom(\bb{F}_2[8], I_\bullet)\to 0,\]
%and thus 
the long exact sequence below.
...
\end{tikzcd}\]
Note that for \(t-s<7\), the map \(\hat i\) must be injective, since \(\Ext^{s,t-8}(\bb{F}_2, \bb{F}_2)=0\). In particular, \(\hat i(h_0), \hat i(h_2)\neq 0\). 
...
Figure \ref{1304251243} shows us that \(\Ext^{3,14}(\bb{F}_2, \bb{F}_2)=0\), so \(\hat i\) is injective at this point, and thus \(\hat i(f_0)\neq 0\). Similarly, \(\Ext^{4,15}(\bb{F}_2, \bb{F}_2)=0\) and \(\Ext^{0,8}(\bb{F}_2, \bb{F}_2)=0\), so \(\hat i(h_0f_0), \hat i(h_4)\neq 0\). Since \(\hat i\) respects multiplication (by \ref{2504211139}), \(\hat i(h_0f_0)=\hat i(h_0)\hat i(f_0)\neq 0\), so \(\hat i(f_0)\) is equal to either \(\overline{\overline{h_0^2h_3}}\) or \(\overline{\overline{h_0^2h_3}}+\hat i(h_2)\hat{i}(h_4)\). In either case, by linearity of \(d_2\), we have \(d_2(\hat i(f_0))=d_2(\overline{\overline{h_0^2h_3}})\) (since \(d_2(\hat i (h_2)\hat i (h_4))=\hat i(d_2(h_2h_4))=0\)\footnote{\textcolour{teal}{Explain: \(d_2(h_2h_4)=0\) because if it wasn't it would be \(e_0\). But \(d_2(e_0)\neq 0\), so this would contradict the fact that \(d_2\) is a differential.}}).

Now, \(\hat i(h_0^2e_0)\neq 0\), since \(\Ext^{5,15}(\bb{F}_2, \bb{F}_2)=0\) (using the long exact sequence in \(\Ext\) again). Thus, \(\hat i(h_2)d_2(\overline{\overline{h_0^2h_3}})= d_2(\hat i(h_2)\overline{\overline{h_0^2h_3}})=
d_2(\hat i(f_0))=\hat i(d_2(f_0))=\hat i(h_0^2e_0)\neq 0\), as required. \done
\end{ourproof}

%\begin{itemize}
%\item \textcolour{violet}{\(h_2 \cdot %\overline{\overline{h_0^2h_3}}=i(f_0)\).} 

%\item \(d_2(f_0)=h_0^2e_0\).
%\item \(i(h_0^2e_0)\neq 0\). [Assuming the long exact sequence I wrote down is correct, we'd have an exact sequence
%\[\Ext^{5,15}(\bb{F}_2, \bb{F}_2)\to \Ext^{6,23}(\bb{F}_2, \bb{F}_2)\xrightarrow{i}\Ext^{6,23}(C \sigma, \bb{F}_2)\]
%But \(\Ext^{5,15}(\bb{F}_2, \bb{F}_2)=0\), so \(i\) is injective at this point. Since \(h_0^2e_0\in \Ext^{6,23}\), we have \(i(h_0^2e_0)\neq 0\).]
%\item Thus(!!) \(d_2(\overline{\overline{h_0^2h_3}})=i(h_0d_0)\). [If we accept the claims that \(h_2\overline{\overline{h_0^2h_3}}=i(f_0)\) and \(i(h_0^2e_0)\neq 0\), and further suppose that \(i\) and \(d_2\) commute [follows from \ref{2504141019}], we have \(h_2d_2(\overline{\overline{h_0^2h_3}})= d_2(h_2\overline{\overline{h_0^2h_3}})=
%d_2(i(f_0))=i(d_2(f_0))=i(h_0^2e_0)\neq 0\).]
%\item ``This intervenes before the image of \(d_3(h_0h_4)=h_0d_0\)''. [I think the point here is just that \(h_0h_4\) and \(h_0d_0\) still exist here, because we're still on the \(E_2\) pages of both spectral sequences.]
%\item Questions: \begin{enumerate}
%\item What's that double bar notation?
%\item \(i : \bb{S} \to C \sigma\). Why does it induce a map on the \(E_2\) pages? Does any map \(X\to Y\) induce a map \(\Ext_{\A}^{s,t}(H^*(X), \bb{F}_2)\to \Ext_{\A}^{s,t}(H^*(Y), \bb{F}_2)\)? Actually, we'd need it to at least induce a map on the \(E_3\) pages too, since we take a \(d_2\) differential below...

%Also, as long as the map on Ext commutes with \(d_2\) (which is what we need anyway!), it'll give a map on \(E_3\). 

%\item Why is \(h_2\) suddenly in \(E_2(C \sigma)\)? Is it \(i(h_2)\)? Why is that nonzero? [Vague idea: we have this exact sequence
%\[\pi_{n-7}(S) \xrightarrow{\sigma}\pi_n(S) \xrightarrow{i} \pi_n(C \sigma) \xrightarrow{j}\pi_{n-8}(S),\]
%but for \(n<7\) we have \(\pi_{n-7}(S)=[\bb{S}, \bb{S}^{7-n}]\). Any map \(\bb{S}\to \bb{S}^{7-n}\) is given by a map \(S^m \to S^{7-n+m}\), which will be homotopy equivalent to a constant map. In other words, we have an exact sequence
%\[0 \to \pi_n(S) \xrightarrow{i}\pi_n(C \sigma) \to 0,\]
%or, in other words, \(i\) is an isomorphism in this range. As a happy coincidence, for \(t-s<7\) there are no differentials, which is why the \(E_2\) pages for \(S\) and \(C \sigma\) look identical in this range. It's also why \(h_2\) shows up, though I still think it should be \(i(h_2)\).

%Obviously this is a bit sketchy, I'm using some sort of ``\(i\) and the differentials play nicely'' assumption.]
%\end{enumerate}
%\end{itemize}
...
\item Is anything lost/does anything become false if I replace every instance of the word `free' with `projective'?
\end{itemize}
...
\item Fix Section \ref{2504171922}.
\end{itemize}

...
At the bottom of p592 of \autocite{hatcher5}, it's mentioned that for a spectrum \(X\) of finite type, \(H^i(X)\cong \lim\limits_{\leftarrow n}H^{i+n}(X_n)\) as \(\mathscr{A}_2\)-modules. Thus, assuming \(\bb{S}\) is of finite type (it is actually finite, since it takes the place of \(Y\) when constructing the Adams spectral sequence, but I should justify this), we have
\begin{align*}

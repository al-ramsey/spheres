...
\begin{definition}
A \textit{spectral sequence} (of \textit{cohomological type})\footnote{I think(?) I actually want one of homological type. I'll check this.} is a collection of differential bigraded \(R\)-modules \(\{E^{*, *}_r, d_r\}, r \in \nat\), with the differentials \(d_r\) of bidegree \((r, 1-r)\). These satisfy the further condition that for all \(p, q, r\), \(E^{p, q}_{r+1}\cong H^{p, q}(E_r^{*, *}, d_r)\).
\end{definition}
...
[Check this is actually the same staircase diagram that Hatcher is talking about]
...
[Some miscellaneous and vague rambling:

\autocite{hatcher5} Prop 5.47 says that there is a spectral sequence \(\{E_r, d_r\}\) such that \(E_2^{s,t}=\Ext_{\mathscr{A}_2}(H^*(\bb{S}), H^*(\bb{S}))\), and \(\{E_r, d_r\}\implies \pi^S_*\) modulo any odd torsion.
...
Lemma 5.49 says that if I have a minimal free resolution 
\[\cdots \to F_2 \to F_1 \to F_0 \to \bb{F}_2 \to 0,\]
where the \(F_i\) are \(\mathscr{A}_2\)-modules, then \(\Ext^{s,t}_{\mathscr{A}_2}(\bb{F}_2, \bb{F}_2)=\Hom^t_{\mathscr{A}_2}(F_s, \bb{F}_2)\), where the left hand side is the group of \(\mathscr{A}_2\)-module homomorphisms \(F_s \to \bb{F}_2\) which lower the degree by \(t\). Now, since \(\bb{F}_2\) just has stuff in degree 0, the only things that can be sent to \(1 \in \bb{F}_2\) are the things in degree \(t\), so for every generator for \(F_s\) in degree \(t\), there's an \(\bb{F}_2\)'s worth of such homs.]
...
Some slightly less cryptic but irritatingly general and opaque comments are made in \autocite{cobordism}, in particular stemming from Lemma 3.1.3. If I just believe this lemma, assuming its \(a_0\) is the same as my \(\alpha^1_1\), and further believe the calculations done in \autocite{ass} in 6.2, then (I think!) this solves my problems. 

\subsection{Nontrivial differentials}

[The point here is that (I claim) all differentials interacting with \(E^{s,t}\) for \(t-s=14\) are trivial, and thus computing the stable homotopy groups is purely mechanical, because everything that appears on this part of the \(E_2\) page has to survive to \(E_\infty\). Thus, the `ambiguity' at \(t-s=14\) is just the fact that this is the first time you need to actually compute differentials.
...
\item The suspension of a pointed cell complex \((X, x_0)\) is another pointed cell complex \(\Sigma X\) with basepoint \(x_0\) and an \(n\)-cell for each non-basepoint \(n-1\) cell \(e^{n-1}_\alpha\) of \(X\).
\end{itemize}
...
\item On p592 of \autocite{hatcher5}, he says that ``for spectra \(X\) of finite type [the more general] definition of an \(\A\)-module structure on \(H^*(X)\) agrees with the definition using the usual \(\A\)-module structure on the cohomology of spaces and the identification of \(H^*(X)\) with the inverse limit \(\lim\limits_{\leftarrow}H^{*+n}(X_n)\)''. Absolutely everything relevant to spheres in the construction of the Adams spectral sequence seems to only use spectra of finite type, so why does Hatcher say on p585 that we can't just take the inverse limit because it doesn't work for the `more general spectra' used when constructing the Adams spectral sequence? Is that just because I could take the role of \(\bb{S}\) and give it to some non-finite-type spectrum (actually, it cannot be this!! Hatcher specifically says the spectrum \(Y\) has to be finite), or is there something I'm missing? Can I just take limits and be done with it?

\item Suppose I have the \(E_2\) page for the appropriate spectral sequence. That in principle only gives me the `dots' and possibly the differentials if I've been clever. But if I just want to calculate, say, \(\pi^S_3\), I get \(\inte_2\) summands at \(E^{1, 4}_2, E^{2, 5}_2,\) and \(E^{3, 6}_2\). So, that says I have a filtration 
...
\item Be consistent with \(\bb{F}_2\), \(\inte_2\), or \(\inte/2\inte\) (I'm trying to use the latter, but old habits die hard).
\end{itemize}

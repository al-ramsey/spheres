...
\begin{definition}
Let \(X=\{X_n\}\) be a CW spectrum. A \textit{subspectrum} of \(X\) is a sequence of subcomplexes \(\{X_n'\subq X_n\}\) satisfying \(\Sigma X_n'\subq X_{n+1}'\)
\end{definition}

\begin{definition}
Let \(X=\{X_n\}\) be a CW spectrum, and \(X'=\{X_n'\}\) a subspectrum of \(X\). Then \(x'\) is \textit{cofinal} if, for each \(n\) and each cell \(e^i_\alpha\) of \(X_n\), the cell \(\Sigma^k e_\alpha^i\) belongs to \(X'_{n+k}\) for all sufficiently large \(k\).
\end{definition}
...
\begin{definition}
Let \(X\) be a pointed topological space. The \textit{reduced suspension} \(\Sigma X\) is the space \(SX/\sim\), where \([x_0, i]\sim [x_0, i']\) for all \(i,i'\in I\). 
\end{definition}

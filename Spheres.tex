...
\item \textcolour{teal}{Lemma: Given a cofibration}
\[\textcolour{teal}{A \xinj{f} X \xrightarrow{i} Cf \xrightarrow{j} \Sigma A \to \cdots,}\]
\textcolour{teal}{where \(X, A\) are of finite type, we get a long exact sequence}
\[\textcolour{teal}{\cdots H^{n-1}(\Sigma A) \leftarrow H^n(X) \xleftarrow{i^*} H^n(Cf) \xleftarrow{j^*} H^n(\Sigma A)\leftarrow H^{n+1}(X) \leftarrow \cdots.}\]
\end{itemize}
]
...
\end{figure}
\footnotetext{By the way, \(f_0\) is the node on the right, i.e. the node at (18,4,2).}
...
\begin{lemma}[{\autocite{rognes2}, Table 14.1 (9)}]
\(d_2(f_0)\neq 0\).
\end{lemma}
...
\[\bb{S}^7 \xrightarrow{\sigma} \bb{S} \xrightarrow{i} C \sigma \xrightarrow{j} \bb{S}^8\to \cdots.\]
Let \(\overline{\overline{h_0^2h_3}}\in E_2^{3,18}(C \sigma)\) be the generator shown in Figure \ref{2504131812}. Then \(d_2(\overline{\overline{h_0^2h_3}})=\hat i(h_0d_0)\), where \(\hat i : \Ext^{s,t}(\bb{S})\to \Ext^{s,t}(C \sigma)\) is the map induced by filling in the diagram below with the unique dotted lifts.\footnote{Make this explanation make any sense.}
\[\begin{tikzcd} 
 \cdots \arrow[r, ""] & F_2 \arrow[r, ""] \arrow[d, "", dashrightarrow] & F_1 \arrow[d, "", dashrightarrow] \arrow[r, ""] & F_0 \arrow[d, "", dashrightarrow] \arrow[r, ""] & H^*(C \sigma) \arrow[d, "i^*"]\\ 
 \cdots \arrow[r, swap, ""]  & P_2 \arrow[r, ""'] & P_1 \arrow[r, ""'] & P_0 \arrow[r, ""'] & H^*(\bb{S})
 \end{tikzcd}\] 
\end{lemma}

\begin{itemize}
\item \textcolour{violet}{\(h_2 \cdot \overline{\overline{h_0^2h_3}}=i(f_0)\).} %[After extended research (and a hint from \autocite{rognes2} Ex 11.6.4) I have deduced that \(n_m\) is just notation for the \(m\)th generator (however that's counted) in row \(n\). So I will be ignoring it.

%Note: it would be lovely if \(i\) as a map \(E_2(S) \to E_2(C \sigma)\) was a ring homomorphism, or an algebra morphism, or whatever.

%Possible lead: on p124 of \autocite{ass}, it says ``Associated to the [cofibre] sequence \(S \to S/2 \to \Sigma S=S^1\), we have an extension \(0 \to \Sigma \bb{F}_2 \to H^*(S/2)\to \bb{F}_2 \to 0\) and a long exact sequence of \(\Ext\)-groups \(\cdots \to \Ext^{s,t}_{\A}(\bb{F}_2, \bb{F}_2)\to \Ext^{s,t}(H^*(S/2), \bb{F}_2)\to \Ext^{s, t-1}_{\A}(\bb{F}_2, \bb{F}_2)\xrightarrow{\delta}\Ext^{s+1,t}_{\A}(\bb{F}_2, \bb{F}_2)\to \cdots\) where the connecting homomorphism is given by the Yoneda product with \(h_0\in \Ext^{1,1}(\bb{F}_2,\bb{F}_2)\)''. A long exact sequence on \(\Ext\) groups would be really useful to me! Because then at least showing \(i(f_0)\neq 0\) would be the same as showing \(f_0\) is not in the image of some boundary map...

%Note that I can't just naively steal that long exact sequence, because \(i(h_3)=0\) but \(h_3\) is a generator (i.e. not the product of something with \(h_0\)). But! Maybe I can make my own long exact sequence.

Claim 1: From the cofibration
\[\bb{S}^7 \xrightarrow{\sigma} \bb{S}\xrightarrow{i} C \sigma \xrightarrow{j}\bb{S}^8,\]
we get a long exact sequence
\[\cdots \leftarrow H^{n-1}(\bb{S}^8) \leftarrow H^n(\bb{S}) \xleftarrow{i^*} H^n(C \sigma) \xleftarrow{j^*} H^n(\bb{S}^8)\leftarrow H^{n+1}(\bb{S}) \leftarrow \cdots\]

[Should follow from the fact that it's true for CW complexes and all these spectra are of finite type]

But any map \(H^n(\bb{S})\to H^{n-1}(\bb{S}^8)\) must be zero, so really we get short exact sequences 
\[0 \leftarrow H^n(\bb{S})\xleftarrow{i^*} H^n(C \sigma) \xleftarrow{j^*} H^n(\bb{S}^8)\leftarrow 0.\]
Summing these up gives a short exact sequence
\[0 \leftarrow H^*(\bb{S})\xleftarrow{i^*} H^*(C \sigma) \xleftarrow{j^*} H^*(\bb{S}^8)\leftarrow 0,\]
i.e.
\[0 \leftarrow \bb{F}_2 \xleftarrow{i^*} H^*(C \sigma) \xleftarrow{j^*} \Sigma^8 \bb{F}_2 \leftarrow 0.\]

Claim 2: From the short exact sequence above, we get a long exact sequence \(\cdots \to \Ext^{s-1,t-8}(\bb{F}_2, \bb{F}_2) \to \Ext^{s,t}(\bb{F}_2, \bb{F}_2) \xrightarrow{i} \Ext^{s,t}(H^*(C \sigma), \bb{F}_2) \xrightarrow{j} \Ext^{s,t-8}(\bb{F}_2, \bb{F}_2)\to \Ext^{s+1,t}(\bb{F}_2, \bb{F}_2)\to \cdots\)

[This is a homological algebra fact]

Now, \(f_0\in \Ext^{4,22}(\bb{F}_2, \bb{F}_2)\), so we look at the exact sequence
\[\Ext^{3,14}(\bb{F}_2, \bb{F}_2)\to \Ext^{4,22}(\bb{F}_2, \bb{F}_2) \xrightarrow{i} \Ext^{4,22}(H^*(C \sigma), \bb{F}_2)\]
Anything in \(\Ext^{3,14}\) would be at coordinate \((11, 3)\) in Figure \ref{1304251243}, but that's just zero. So the kernel of \(i\) at this point is zero, so \(i(f_0)\neq 0\). Similarly, \(\Ext^{4,15}=0\) so \(i(h_0f_0)\neq 0\). If \(i\) is an algebra morphism\footnote{I've just drawn some diagrams, and I think that yes, \(i(\alpha \beta)=i(\alpha)i(\beta)\). Essentially because the way you multiply in \(\Ext\) involves writing down unique lifts that make certain squares commute, but both \(i(\alpha \beta)\) and \(i(\alpha)i(\beta)\) are lifts making the appropriate squares commute, so by uniqueness they must be the same.}, then \(i(h_0f_0)=i(h_0)i(f_0)\neq 0\), so \(i(f_0)\) cannot be at \((18,4,1)\), it must actually be at \((18,4,2)\) (well, actually it could be the sum of the two generators, but really all I need is for \(d_2\) of it to be \(d_2((18,4,2))\), which it will be since \(d_2((18,4,1)+(18,4,2))=d_2((18,4,1))+d_2((18,4,2))=d_2((18,4,2))\)). The claim \(h_2\overline{\overline{h_0^2h_3}}=i(f_0)\) would then follow.]
...
\item Thus(!!) \(d_2(\overline{\overline{h_0^2h_3}})=i(h_0d_0)\). [If we accept the claims that \(h_2\overline{\overline{h_0^2h_3}}=i(f_0)\) and \(i(h_0^2e_0)\neq 0\), and further suppose that \(i\) and \(d_2\) commute [which they do, because \(i\) is a chain map and \(d_2\) is a differential], we have \(h_2d_2(\overline{\overline{h_0^2h_3}})= d_2(h_2\overline{\overline{h_0^2h_3}})=
...
\caption{The \(E_2\) page of the Adams spectral sequence for \(C \sigma\), in the range \(t-s\leq 18\), with the generator \(\overline{\overline{h_0^2h_3}}\) shown in red.}
\end{figure}
...
%Found something useful!! Not sure where to put it though.

%\begin{theorem}[{\autocite{ass}, Cor 4.17}]
...
\begin{itemize}
\item In \(E_2(C \sigma)\) we have \(d_2(\overline{\overline{h_0^2h_3}})=i(h_0d_0)\) (by \ref{2504131851}), so \(\pi_{14}(C \sigma)^\wedge_2\) has order dividing four.

\item \(\pi_6(S)^\wedge_2=\inte/2\inte \ang{\nu^2}\). [Assuming \(\nu\) is the thing represented by \(h_2^2\)]
...
\item \(\pi_7(S)^\wedge_2=\inte/16\inte \ang{\sigma}\). [All assuming \(h_3\) detects \(\sigma\).]

\item \(2 \sigma^2=0\) by graded commutativity.

\item \(\pi_{14}(S)^\wedge_2\) has order dividing four. [First isomorphism theorem.]

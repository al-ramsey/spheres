...
\[\begin{tikzcd}
 ...
 \end{tikzcd}\] 
But that's what we'd have gotten if we took \(X \xrightarrow{fg}Z\) and made it into a map \(X \to Z^\wedge_p\) directly. So it seems like the composition product 
\[[Y, \text{}_2Z]_0\otimes [X, \text{}_2Y]_0\to [X, \text{}_2Z]_0,\]
induces the one 
\[[Y, Z]_0\otimes [X, Y]_0\to [X, Z]_0,\]
which is \textit{really weird}, because \([Y,Z]\) could easily be (and often is) bigger than \([Y, \text{}_2Z]\). But maybe this is just an artefact of me having restricted to degree 0? Does this argument still work when I consider general degrees? I find it really difficult to believe I could figure out the product on odd torsion parts of \(\pi_i^S\) from just the even parts, for example. 
\end{enumerate}
\end{itemize}

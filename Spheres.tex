...
\begin{definition}
Let \(f : X \to Y\) be a map of CW spectra, and let \(f' : X' \to Y\) be a representative for \(f\), where \(X'\subq X\) is cofinal. The \textbf{mapping cylinder} \(M_f\) has components \((M_f)_n=M_{f'_n}\), where \(M_{f'_n}\) is the reduced mapping cylinder of \(f'_n\), and is independent of the choice of \(X'\) up to equivalence.
\end{definition}
...
\textcolour{teal}{[Some intro. Following \autocite{spectral_sequences}, \autocite{hatcher5}, \autocite{weibel}, \autocite{hatcher}. From this point on, all homology and cohomology will be taken with \(\bb{F}_2\) coefficients, and we will thus ease notation by writing \(H^*(X)\) (resp. \(H_*(X)\)) for \(H^*(X, \bb{F}_2)\) (resp. \(H_*(X;\bb{F}_2)\)). ]}
...
We can replace that map with an inclusion, by \ref{2504231152}.
...
Continuing in this fashion, the computations for rows 4 and 5 are shown in \ref{2504201106}, though the Adem relations required to justify them are not. Note that although to compute each row, knowledge of maps involving the next two rows is required, rows 6 and 7 do not contain all the new generators needed. 
...
\nathaniel{What I've written below is not quite right - what we actually see is that we have 3 dots on the \(E_2\) page, but we haven't yet shown that there are no differentials in this range. I'll have to move some things around... Also, I need to mention that we know \((\pi_1^S)^\wedge_2\) and \((\pi_2^S)^\wedge_2\), since there's no ambiguity.}
...
be free resolutions for \(L\) and \(M\). Then, given \([g] \in \Ext_A^{s,t}(M, N)\), \([h]\in \Ext_A^{u,v}(L, M)\), we inductively construct a chain map \(h_\bullet : F_{u+\bullet} \to F'_{\bullet}[v]\), as shown in the diagram below.
\[\begin{tikzcd} 

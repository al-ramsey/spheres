...
Thus, the maps \((\sigma_n)_* : \pi_{i+n+1}(\Sigma X_n)\to \pi_{i+n+1}(X_{n+1})\) are also eventually isomorphisms, so the groups \(\pi_{i+n}(X_n)\) do stabilise.
\end{proof}
...
In our case, let \(h_3\in E_2^{1,8}\) be represented by \(\sigma\in (\pi_7^s)^\wedge_2\), and let \(h_1\in E_2^{1,2}\) be represented by \(\eta \in (\pi_1^s)^\wedge_2\). Then \(h_1h_3\) is represented by \(\eta \sigma\), and \(2(\eta \sigma)=(2\eta)\sigma=0\), since \((\pi_1^s)^\wedge_2\cong \inte/2\inte\). Thus, \((\pi_9^s)^\wedge_2\cong (\inte/2\inte)^3\). 
\end{proof}
